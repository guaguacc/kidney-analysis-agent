...
> 我做了一个基于规则引擎和 GPT API 的肾病指标分析 Agent。系统使用 FastAPI 提供分析接口，先对患者的 Scr、eGFR、ACR 和血压等指标进行规则判断，输出风险等级、CKD 分期和异常项，再调用 GPT 生成中文医学解释。同时，我还加入了患者历史记录和趋势分析能力，支持对同一患者多次检测结果进行改善/恶化判断。整个项目采用模块化结构，便于后续继续扩展数据库、前端和知识库。
